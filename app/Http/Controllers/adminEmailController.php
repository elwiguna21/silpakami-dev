...
class adminEmailController extends Controller
{
    public function index()
    {
        ...
        return view('admin.email', compact('instansi'));
    }

    public function aprove(Request $request)
    {
        $id = $request->idaprove;
        $unit = Email::where('id', $id)->update([
            'stat' => "2"
        ]);
        try {
            return redirect()
    ...
    {
        $pengajuan = Email::where('stat', '<=', '5')->get();
        ...
        return Datatables::of($pengajuan)
            ...
            ->addColumn('action', function ($user) {
                ...
                <a href="" data-bs-toggle="modal" data-bs-target="#denied" id="bdenied" class="btn btn-outline-danger" data-id="' . $user->id . '" data-email="' . $user->email . '">Denied</a>';
                    ...
                    return '<a href="" data-bs-toggle="modal" data-bs-target="#published" id="bpublished" class="btn btn-outline-success btn-xs" data-id="'.$user->id.'" data-email="'.$user->email.'">publish</a>';
                }
                else {
                    return '';
                }
